<button command="show-modal" commandfor="updateDialog{{ $item->id_statistik }}"
...
<el-dialog>
    <dialog id="updateDialog{{ $item->id_statistik }}"
            ...
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden transition-all">
                ...
                <form action="{{ route('statistik.update', $item->id_statistik) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex justify-end gap-2 px-6 py-4 bg-gray-100">
                        <button type="button" command="close" commandfor="updateDialog{{ $item->id_statistik }}"
