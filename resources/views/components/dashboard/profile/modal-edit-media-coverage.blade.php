<button type="button"
    onclick="document.getElementById('updateMediaCoverageDialog{{ $item->id_media_coverage }}').showModal()"
    class="text-white/40 hover:text-white transition text-xs leading-none">
    <i class="bi bi-pencil-fill"></i>
</button>
<dialog id="updateMediaCoverageDialog{{ $item->id_media_coverage }}"
    ...
    <div class="min-h-screen w-full flex items-center justify-center p-4">
        ...
        <div
            ...
            <div class="flex items-center justify-between gap-2 px-6 py-4 border-b border-white/10 bg-white/3 shrink-0">
                <h3 class="font-bold uppercase tracking-wide text-sm text-white">Update Media Coverage</h3>
                <button type="button"
                    onclick="document.getElementById('updateMediaCoverageDialog{{ $item->id_media_coverage }}').close()"
                    class="text-white/40 hover:text-white transition">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <!-- Form -->
            <form action="{{ route('media_coverage.update', $item->id_media_coverage) }}" method="POST"
                ...
                <div class="flex-1 min-h-0 p-6 flex flex-col gap-4 overflow-y-auto">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="nama_media" value="{{ $item->nama_media }}"
